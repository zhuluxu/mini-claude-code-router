...
规则的本质是"什么任务走什么模型"。要配得好，关键是搞清楚两类事情：**规则怎么写**（识别什么任务）和 **模型怎么选**（匹配什么能力）。
...
判断依据：没有工具调用（不是 agentic 任务），对话很短（一问一答）。走最便宜的模型，回答"这个变量什么意思"这种问题绰绰有余。
...
判断依据：有工具调用（要读写文件、跑命令），对话有一定长度（多轮交互的 agentic 任务）。走平衡档，代码能力和工具调用都靠得住，成本可控。
...
判断依据：Claude Code 自动加了 `thinking`（模型支持深度思考），同时有工具且对话已很长（复杂的多步 agentic + 推理任务）。这种最费脑，走强力档。
...
判断依据：有 thinking 但没有工具，对话很短（用户开了思考模式但只是问个稍复杂的问题）。平衡档足够，不用上强力档浪费钱。
...
判断依据：有 thinking 且对话很长（多轮深度讨论，可能在做架构设计或方案论证）。上下文长 + 推理深，需要强力档的长上下文能力和推理质量。

#### 规则顺序很重要

规则按数组顺序匹配，命中第一个就停止。**越具体的规则越靠前，越宽泛的越靠后**。推荐顺序：
...
最后用 `defaultModel` 兜底所有没命中的请求——通常设为平衡档，保证未匹配的请求也能正常处理。
...
配置 provider 时需要声明 `type`，告诉网关这个上游用的是什么协议：

| type | 适用场景 | 网关行为 |
| --- | --- | --- |
| `anthropic_messages` | Anthropic 官方 API，或任何原生兼容 Anthropic Messages 协议的端点 | 同协议透传，只替换 model 和鉴权，不改动请求体 |
| `openai_chat_completions` | DeepSeek、通义千问、mimo、OpenRouter 等 OpenAI 兼容端点 | 自动协议转换，Anthropic 请求体转 OpenAI 格式，响应再转回来 |
| `openai_responses` | OpenAI Responses API（实验性） | 协议转换，当前复用 Chat Completions 转换逻辑 |
| `gemini_generate_content` | Google Gemini API | 仅替换鉴权和 model，不做格式转换 |

**怎么判断该选哪个？** 看你的上游 API 文档：
...
大多数第三方模型服务（DeepSeek、通义、mimo 等）都走 `openai_chat_completions`，这是兼容性最广的类型。如果你用的是某个聚合平台（如 OpenRouter），它通常同时支持多种协议，优先选 `anthropic_messages`（同协议透迵不丢字段），不行再换 `openai_chat_completions`。
